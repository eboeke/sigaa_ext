function acq(subjectcode,eyetrack,screenStuff)
% runs acquisition phase of experiment
%args: 
...
try
    ...
    components = regexp(subjectcode,regexptranslate('escape','_'),'split');
    ...
    inst_1 =['In this study you are going to see two faces.\n\n'...
        'Press the space bar to continue.'];
    inst_2 =['Here are the two faces.\n\n\n\n\n\n\n\n\n\n'...
        'Press the space bar to continue.'];
    inst_3 = ['One face, the threat face, will sometimes be\n'...
        'paired with shock. The other face, the no-threat\n'...
        'face, will never be paired with shock. \n\n'...
        'Press the space bar to continue.'];
    ...
    end_text = ['Time for a break. You can lean back in the chair. \n'...
    ...
    if order==1
        ...
        %set stim order
     acq.acq_mtx(:,cstype_idx) = [CSpu_val CSm_val CSp_val CSpu_val CSm_val CSm_val CSp_val...
    ...
    elseif order==2
        ITI_durs = [10 12 10 8 12 12 8 12 10 8 12 10 10 12 8 10 10 8 10 10 12 12 10 8 8 10 8 12 10 8 12 8];

        acq.acq_mtx(:,cstype_idx)=  [3 1 2 3 1 2 1 3 1 2 2 3 2 1 1 1 2 3 2 1 3 2 3 2 1 1 2 3 1 1 2 2]; 
    end
    acq.acq_mtx(1,iti_offset_idx) = initial_ITI_dur + trial_dur + ITI_durs(1);%calculate time after first trial + ITI afterwards elapses
    acq.acq_mtx(1,cs_offset_idx) =initial_ITI_dur+trial_dur; %calculate time after first trial elapses 

    %set trial timing
    for x = 2:nTrials %makes a vectors of times that trials and ITIs should end, with respect to the start of the first ITI.
        acq.acq_mtx(x,cs_offset_idx) = acq.acq_mtx(x-1,cs_offset_idx)+ ITI_durs(x-1)+ trial_dur ;
        acq.acq_mtx(x,iti_offset_idx) = acq.acq_mtx(x-1,iti_offset_idx) + trial_dur + ITI_durs(x);
    end
    ...
    results_mat_file = fullfile(results_dir,sprintf('%s_acq_task.mat',subjectcode)); %name matlab file
    results_txt_file = fullfile(results_dir,sprintf('%s_acq_task.txt',subjectcode)); %name text file
    ...
    for x = 1:nTrials 
        acq.acq_mtx(x,trial_idx) = x; % set the trial number

        Screen('DrawTexture', window,eval(CSmap{acq.acq_mtx(x,cstype_idx)}),[], position_cs, 0); %display cs picture
        Screen(window, 'Flip');
        %***************************
        io64(object,address,1); %send marker to acqknowledge
        %***************************
        acq.acq_mtx(x,cs_onset_act_idx) = GetSecs - exp_onset; % record the onset time of this grid trial
        %send event marker to eyelink
        if (eyetrack)
            Eyelink('Message', [CSmap{acq.acq_mtx(x,cstype_idx)} '_on']);
        end
        if(acq.acq_mtx(x,cstype_idx)==CSpu_val)
            WaitSecs('UntilTime',(exp_onset+acq.acq_mtx(x,cs_offset_idx)-shock_duration));
             %***************************
            io64(object,address,100); %send shock         
            io64(object,address,1);
              %***************************
            acq.acq_mtx(x,shocked_idx) = 1; %record info ab shock
            acq.acq_mtx(x,shock_onset_idx) = GetSecs-exp_onset;
            WaitSecs('UntilTime',exp_onset+acq.acq_mtx(x,cs_offset_idx)) ;
        else
            WaitSecs('UntilTime',exp_onset+acq.acq_mtx(x,cs_offset_idx)) ;
        end

        acq.acq_mtx(x,cs_offset_act_idx) = GetSecs - exp_onset; %record cs offset time
         %***************************
        ...
        acq.acq_mtx(x,iti_onset_act_idx) = ITI_onset - exp_onset;
        %mark end of trial
        if eyetrack
            Eyelink('Message', [CSmap{acq.acq_mtx(x,cstype_idx)} '_off']);
        end

        while (GetSecs < exp_onset + acq.acq_mtx(x,iti_offset_idx)) %while ITI time (minus anticipation time) has not elapsed
            [~,~,keyCode]=KbCheck;
            keyCodeNum = find(keyCode==1);
            if (keyCodeNum == quit_key) %check for quit key, quit if pressed
                Screen('closeall');
        ...
        acq.acq_mtx(x,iti_offset_act_idx) = GetSecs - exp_onset; %record ITI offset time
        
        %print tab delimited results from this trial to text file
        fprintf(fid,'%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n', ...
            acq.acq_mtx(x,:));

        save(results_mat_file, '-struct', 'acq'); %save mat file
    end
